...
const ALLOWED_ORIGINS = new Set(['https://www.shikhasolutions.com','https://shikha-portfolio-blush.vercel.app']);
...
async function send(apiKey,payload){const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.message||'Email delivery failed');return result}

module.exports=async function handler(req,res){
  res.setHeader('Cache-Control','no-store');
  if(req.method!=='POST'){res.setHeader('Allow','POST');return res.status(405).json({ok:false,error:'Method not allowed'})}
  const origin=req.headers.origin;if(origin&&!ALLOWED_ORIGINS.has(origin))return res.status(403).json({ok:false,error:'Origin not allowed'})
  ...
  if(clean(body.website))return res.status(200).json({ok:true,customerEmailSent:false,ownerEmailSent:false})
  ...
  if(!name||!validEmail(email)||!phone||!stage||!consent||overall===null||categories.length!==6||topGaps.length<1)return res.status(400).json({ok:false,error:'Please complete all required fields.'})

  const safe={name:escapeHtml(name),email:escapeHtml(email),phone:escapeHtml(phone),company:escapeHtml(company||'Not provided'),stage:escapeHtml(stage)}
  ...
  const nextSteps=`<h2 style="color:#061A33;font-family:Georgia,serif;font-size:20px;">Choose your next step</h2><p><strong>Need clarity?</strong> Book a Growth Discovery Call.</p><p><strong>One specific measurable issue?</strong> Explore the 7-Day Growth Sprint.</p><p><strong>Several connected execution gaps?</strong> Explore the Monthly Growth Retainer.</p>`
  const customerHtml=`<!DOCTYPE html><html><body style="margin:0;background:#F7F3EC;font-family:Arial,Helvetica,sans-serif;color:#20252C;"><table width="100%" cellpadding="0" cellspacing="0" bgcolor="#F7F3EC"><tr><td align="center" style="padding:24px;"><table width="620" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="width:100%;max-width:620px;border-top:6px solid #C98A16;"><tr><td bgcolor="#061A33" style="padding:28px 30px;"><p style="margin:0 0 7px;color:#E2AA43;font-size:12px;font-weight:bold;letter-spacing:1px;">SHIKHA SOLUTIONS</p><h1 style="margin:0;color:#FFFFFF;font-family:Georgia,serif;font-size:28px;">Your Business Health Result</h1></td></tr><tr><td style="padding:28px 30px;font-size:14px;line-height:1.65;"><p>Hello ${safe.name},</p><p>Your overall Business Health Score is:</p><p style="font-size:34px;color:#C98A16;font-weight:bold;margin:8px 0;">${overall}/100</p><h2 style="color:#061A33;font-family:Georgia,serif;">${descriptor(overall)}</h2><h2 style="color:#061A33;font-family:Georgia,serif;font-size:20px;">Six category scores</h2><table width="100%" cellpadding="0" cellspacing="0">${categoryRows}</table><h2 style="color:#061A33;font-family:Georgia,serif;font-size:20px;">Your top three gaps</h2><ol style="padding-left:20px;">${gapRows}</ol>${alertRows}${nextSteps}<p style="margin-top:26px;"><a href="https://calendar.app.google/due6sDbM5aywr4Uu8" style="display:inline-block;background:#061A33;color:#FFFFFF;text-decoration:none;font-weight:bold;padding:12px 18px;border-radius:5px;">Book a Growth Discovery Call →</a></p><p style="margin-top:12px;"><a href="https://www.shikhasolutions.com/7-day-growth-sprint" style="color:#061A33;font-weight:bold;">Explore the 7-Day Growth Sprint</a> &nbsp;·&nbsp; <a href="https://www.shikhasolutions.com/monthly-growth-retainer" style="color:#061A33;font-weight:bold;">Explore the Monthly Growth Retainer</a></p><p style="font-size:12px;color:#65707b;margin-top:24px;">This is a research-informed business diagnostic, not an audit or certification.</p></td></tr></table></td></tr></table></body></html>`
  const ownerHtml=`<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;color:#20252C;"><h1>New Business Health Assessment lead</h1><p><strong>Name:</strong> ${safe.name}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Phone:</strong> ${safe.phone}</p><p><strong>Company:</strong> ${safe.company}</p><p><strong>Business stage:</strong> ${safe.stage}</p><p><strong>Overall score:</strong> ${overall}/100 · ${descriptor(overall)}</p><h2>Top gaps</h2><ol>${gapRows}</ol>${alertRows}</body></html>`

  const [customer,owner]=await Promise.allSettled([
    send(apiKey,{from:FROM,to:[email],reply_to:OWNER,subject:`Your Shikha Solutions Business Health Result: ${overall}/100`,html:customerHtml,tags:[{name:'source',value:'business-health-assessment'}]}),
    send(apiKey,{from:FROM,to:[OWNER],reply_to:email,subject:`New assessment lead: ${name}${company?` · ${company}`:''} · ${overall}/100`,html:ownerHtml,text:`New Business Health Assessment lead\n\nName: ${name}\nEmail: ${email}\nPhone: ${phone}\nCompany: ${company||'Not provided'}\nBusiness stage: ${stage}\nOverall score: ${overall}/100\nTop gaps: ${topGaps.map(x=>`${x.cat} ${x.score}/100`).join(', ')}\nAlerts: ${alerts.join(', ')||'None'}`,tags:[{name:'source',value:'business-health-assessment-owner'}]})
  ])
  if(customer.status==='rejected')console.error('Customer assessment email error',customer.reason)
  if(owner.status==='rejected')console.error('Owner assessment notification error',owner.reason)
  return res.status(200).json({ok:true,customerEmailSent:customer.status==='fulfilled',ownerEmailSent:owner.status==='fulfilled'})
}
